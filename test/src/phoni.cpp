...
void run(const Args& args) {
  ...
  for(size_t patternid = 0; patternid < patterndescs.size(); ++patternid){
    ...
    t_insert_start = std::chrono::high_resolution_clock::now();
        const size_t patternlength = ms.query_mum(patternfilename, std::string(args.filename) + ".mums");
        // const size_t patternlength = ms.query_mum(patternfilename, std::string(args.filename) + ".binrev.length",  std::string(args.filename) + ".binrev.pointers", patternfilename + ".MUM_pos_reference", patternfilename + ".MUM_pos", patternfilename + ".MUM_len");
